...
function createNewTrackObject(mediaInfoObj){
	...
	var genreName = (mediaInfoObj.genre || "Unbekannt");
	var genreId = addGenre(genreName);
	...
	var track = {
			...
			album: (mediaInfoObj.album || "???")
		    };
	
	addArtist(track.artist);
	addAlbum(track.artist,track.album);
...
function addAlbum(artistName,albumName){
	...
	if(albumId == albums.length)
		albums.push({id : albumId, artistId: artistId, name : albumName });
}
